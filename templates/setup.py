######################################################################################
#                                                                                    #
#  template for setup.py file for building Cython modules in Python projects         #
#                                                                                    #
######################################################################################
...
if len(sys.argv) > 1 and sys.argv[1].startswith("--debug="):
    ...
    try:
        ...
        raise(ValueError("Assignment of DEBUG from system argument did not work."
                         "Aborting cython build!"))
else:
    DEBUG = False
...
# give name of the module to be built here as well as the path to the .pyx file
# and path of module tree, e.g.: module.submodule.name
name = ""
filepath = ""
modulepath = ""
...
if sys.platform == "linux":
    ...
    extra_compile_args.append("-O3", "-fPIC")
elif sys.platform == "darwin":  # macOS
    # -mmacosx-version-min=10.9 ensures compatibility with older macOS versions
    extra_compile_args.extend(["-O3", "-mmacosx-version-min=10.9"])
